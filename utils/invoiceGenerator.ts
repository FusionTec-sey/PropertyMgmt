...
export function generateInvoiceData(params: GenerateInvoiceParams): Omit<Invoice, 'id' | 'tenant_id' | 'created_at' | 'updated_at'> {
  ...
  const rentPeriodText = rentPeriod || formatRentPeriodFromDueDate(dueDate);

  const lineItems: InvoiceLineItem[] = [
    {
      id: '1',
      description: `Monthly Rent - ${rentPeriodText}`,
      quantity: 1,
...
export function generateMonthlyInvoiceDate(lease: Lease): { invoiceDate: string; dueDate: string; rentPeriod: string } {
  ...
  const rentPeriod = formatRentPeriodFromDueDate(dueDate);
  
  return { invoiceDate, dueDate, rentPeriod };
}
...
<body>
  <div class="invoice">
    ...
    <div class="details" style="margin-top: -20px; margin-bottom: 30px;">
      <div class="details-column">
        <h3>Rent Period</h3>
        <p style="font-size: 16px; font-weight: 600; color: #007AFF;">
          ${invoice.recurring_period || formatRentPeriodFromDueDate(invoice.due_date)}
        </p>
      </div>
      <div class="details-column">
        <h3>Due Date</h3>
