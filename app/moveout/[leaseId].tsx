...
import { CheckSquare, Square, ChevronDown, ChevronUp, DollarSign } from 'lucide-react-native';
import Button from '@/components/Button';
import Card from '@/components/Card';
import { MoveOutChecklistItem } from '@/types';
...
export default function MoveOutChecklistScreen() {
  ...
  const [checklistItems, setChecklistItems] = useState<MoveOutChecklistItem[]>(
  ...
  const updateItemCondition = (itemId: string, condition: MoveOutChecklistItem['condition']) => {
    setChecklistItems(items =>
      items.map(item =>
  ...
  const updateItemDamageCost = (itemId: string, cost: number) => {
    setChecklistItems(items =>
      items.map(item =>
        item.id === itemId ? { ...item, damage_cost: cost } : item
      )
    );
  };
  ...
  const totalDamageCost = checklistItems.reduce((sum, item) => sum + (item.damage_cost || 0), 0);
  ...
  if (!lease || !unit || !tenant) {
    return (
      <View style={styles.container}>
        <Stack.Screen options={{ title: 'Move-Out Checklist' }} />
  ...
  return (
    <View style={styles.container}>
      <Stack.Screen
        options={{
          title: `Move-Out Inspection - Unit ${unit.unit_number}`,
        }}
      />
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <Card style={styles.headerCard}>
          <Text style={styles.headerTitle}>Move-Out Inspection Checklist</Text>
          ...
          <View style={styles.headerInfo}>
            <Text style={styles.headerLabel}>Security Deposit:</Text>
            <Text style={styles.headerValue}>₨{depositAmount.toLocaleString()} SCR</Text>
          </View>
        </Card>
        ...
        <Card style={styles.summaryCard}>
          ...
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Total Damage Cost:</Text>
            <Text style={[styles.summaryValue, totalDamageCost > 0 && { color: '#FF3B30' }]}>
              ₨{totalDamageCost.toLocaleString()}
            </Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Deposit Deduction:</Text>
            <Text style={[styles.summaryValue, totalDamageCost > 0 && { color: '#FF3B30' }]}>
              ₨{Math.min(totalDamageCost, depositAmount).toLocaleString()}
            </Text>
          </View>
          <View style={[styles.summaryRow, styles.summaryTotal]}>
            <Text style={styles.summaryTotalLabel}>Deposit Return:</Text>
            <Text style={styles.summaryTotalValue}>
              ₨{depositReturn.toLocaleString()} SCR
            </Text>
          </View>
        </Card>
              ...
              >
                <View style={styles.sectionHeaderLeft}>
                  ...
                  <Text style={styles.sectionProgress}>
                    {progress.checked}/{progress.total} inspected
                  </Text>
                </View>
                {isExpanded ? (
                      ...
                      <View key={itemId} style={styles.checklistItem}>
                        ...
                        {(item.condition === 'poor' || item.condition === 'damaged') && (
                          <View style={styles.damageSection}>
                            <View style={styles.damageCostRow}>
                              <DollarSign size={16} color="#FF3B30" />
                              <Text style={styles.damageCostLabel}>Repair/Replacement Cost:</Text>
                              <TextInput
                                style={styles.damageCostInput}
                                placeholder="0"
                                placeholderTextColor="#999"
                                value={item.damage_cost ? String(item.damage_cost) : ''}
                                onChangeText={(text: string) => {
                                  const cost = parseFloat(text) || 0;
                                  updateItemDamageCost(itemId, cost);
                                }}
                        ...
                        <TextInput
                          ...
                          placeholder="Add notes about condition or damage (optional)"
                          placeholderTextColor="#999"
                          value={item.notes}
        ...
        <View style={styles.actions}>
          <Button
            title="Proceed to Deposit Return"
